...
- 项目处于早期概念验证阶段(PoC)，不需要考虑兼容性，遇到兼容性问题直接使用最终的版本即可
- 所有方法与函数要有完备的测试用例覆盖，要求代码行覆盖率达到>=90%
- 代码中的错误都需要处理，这里的处理意思是当有返回错误的时候需要用变量接受，当确定错误无关紧要，也要使用 _ 来忽略，而不是完全忽略
- 后端代码编写完毕后需要使用golangci-lint和goimports-reviser进行格式化与lint处理
- 后端代码杜绝在for循环中使用defer
...
- git操作时，如果需要commit或者push代码，在处理前必须跑通与 CI 对齐的本地门禁，任一步失败严禁提交或推送。仅跑部分测试/构建不算完成门禁。
...
- 本地个人工作流 skill 可保留私人默认路径、私人笔记目录与本机脚本入口；若对外发布，必须基于单独副本做脱敏，不直接公开 `~/.agents/skills/` 源文件。
